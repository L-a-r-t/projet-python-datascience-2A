...
pas. On n'a parfois qu'une annonce simple, une alerte de trafic qui ressemble peu ou prou à ça :
...
- [Modèle](#model)
  ...
  - [Entraîment et évaluation](#traineval)
- [Résultats](#results)
- [Discussion et conclusion](#discussion)
...
<a name="code"></a>
#### Code
Le code est situé dans le dossier `src`, et sa structure peut être suivie via un notebook principal `main.ipynb`.
...
- le scraping périodique de l'API IDF Mobilités est effectué par une GitHub Action paramétrée dans
  `.github/workflows.scheduler.yml`, qui exécute le fichier `scraping-job.py`. Les réponses brutes de l'API IDF Mobilités
...
- Une partie de l'analyse, notamment l'augmentation des données avec CamemBERT, est effectuée à part dans le notebook
  `analysis.ipynb`. Voir [Analyse des données](#analysis) pour plus de détails.

- L'entraîment et l'évaluation du modèle, particulièrement conséquent, effectués dans le notebook `model.ipynb`. Voir
...
à ce sujet). Ces variables peuvent être stockées dans un fichier `.env` à la racine, suivant les conventions.
...
Les opérations effectuées au cours de ce projet peuvent être assez longues. Pour gagner du temps (ou simplement parce
que les CPU n'étaient parfois pas adaptés) nous avons utilisé des GPU, notamment ceux mis en place par le SSP Cloud.
...
du SSP Cloud, exécuté périodiquement par une GitHub action.

L'action est en théorie censée s'exécuter toutes les deux minutes ainsi que paramétrée par la tâche CRON
(`.github/workflows/scheduler.yml`) et selon ce que permettaient les quotas d'utilisation de l'API IDF Mobilités. En
...
tourner soi-même un worker (ou la tâche CRON directement), mais nous ne disposions pas de serveur pour faire cela.
...
Pour cela, nous avons filtré les doublons, et structuré les données en trois tables principales. Une pour les
...
et sur lesquels il nous était plus rapide d'itérer qu'avec _parquet_.
...
les deux-tiers des lignes de bus du réseau.
...
présence de mots-clés soigneusement choisis à l'intérieur du message d'info-trafic lié, mais celle-ci n'a pas été
efficace.
...
On travaille avec le jeu de données des liens-objets-perturbations. L'objectif est d'entraîner un modèle pour prédire
...
- le type d'objet impacté (colonne `object_type`), ayant quatre modalités : `line`, `stop_point`, `network`, et `stop_area` ;
...
version pré-entraînée `camembert-base` (voir la
...
multi-couche assez simple (simplement deux couches et une ReLU), chargé de prédire en même temps la durée de la
...
nous intéresse), les analyses préliminaires ayant montré qu'il pouvait sinon y avoir beaucoup de valeurs extrêmes.
...
one-hot, et évaluées par une perte d'entropie croisée binaire. Le modèle est entraîné avec une perte correspondant
...
GPU L4. Le notebook est cependant conçu pour être exécuté avec l'environnement jupyter-pytorch-gpu du SSP Cloud, et
...
On observe que plus de 80 % des perturbations se déroulent sur moins d'un jour. On filtre donc
ces perturbations. On va ensuite appliquer une normalisation robuste des colonnes `duration_hours` et `duration_minutes`
sur le jeu qu'on vient de filtrer. On la calcule avec

$$ X_{i,\text{robuste}} = \dfrac{X_i - X_{q,50}}{X_{q,75} - X_{q, 25}} $$

où $X_{q,25}, X_{q,50}, X_{q,75}$ sont le premier, deuxième (médiane) et troisième quartile de $X$ (il existe des
...
Par souci de reproductibilité, le `random_state` a été fixé à une valeur précise (voir le notebook `model.ipynb`).
...
Pour l'entraînement et l'évaluation, on utilise le module `Pytorch` et les utilitaires adaptés. Le détail peut être
trouvé dans le notebook `model.ipynb`. On enregistre dans `checkpoint/epoch_3.pt` les poids du modèle au troisième
epoch, et dans `checkpoint/normalization.json` les valeurs utilisées pour la normalisation. Ainsi, on peut inférer
...
Sur les cinq premiers epochs, on observe que les pertes d'entraînement et de validation sont extrêmement stables, et ce,
...
epoch sont stockés dans `checkpoint/epoch_3.pt`.
...
sont d'environ $0.72$ pour les heures et $0,40$ pour les minutes. Puisque les valeurs ont été traitées par la
normalisation robuste, une telle performance n'est pas particulièrement bonne, et on aurait souhaité mieux.

L'exemple d'inférence donné plus haut semble montrer que le modèle répond des durées qui semblent vraisemblables,
mais rien ne dit que cette prédiction sera juste.

<a name="discussion"></a>
## Discussion et conclusion

De nombreux points peuvent être envisagés pour expliquer la mauvaise performance du modèle :

- Il est possible, tout simplement, qu'il soit difficile de concevoir un tel modèle. Cependant, il peut aussi sembler
  qu'intuitivement, à force d'être confrontés aux retards des moyens de transport, l'on puisse commencer à se faire une
  idée du temps que cela va prendre. La tâche ne paraît donc pas si complexe.

- Un autre point est celui du jeu de données : plusieurs problèmes se sont posés dès le début (incidents qui durent moins
  de deux minutes, GitHub Action non exécutée), et l'analyse des données a par ailleurs montré que de nombreux résultats
  renvoyés par l'API n'étaient pas justes. On aurait sûrement pu laisser tourner le script de scraping plus longtemps, si
  les conditions l'avaient permis.

- Il est possible que le traitement des données en amont du modèle soit très insuffisant. Après considération, il aurait
  été intéressant, plutôt que de découper la durée en jours, heures, minutes, de clusteriser les incidents de durées similaires
  (environ un quart d'heure, une/deux heures, plusieurs heures, une demi-journée, etc.) et d'utiliser un objectif de
  classification.

- Peut-être l'architecture choisie n'est pas adaptée. _BERT_ est basé sur un transformer, et même si l'approche ici
  se rapproche du fine-tuning, il faudrait peut-être beaucoup plus de données, ou plus de données variées. Peut-être aussi
  laisser le modèle calculer plus longtemps ? Il semble ici qu'on soit en situation d'underfit.
  Du point de vue du modèle, de nombreuses choses peuvent être envisagées, ne serait-ce qu'en modifiant
  la fonction de perte.

- Peut-être avons-nous simplement fait une erreur dans notre code ?

En conclusion, il semble y avoir beaucoup de possibilités à explorer, pouvant potentiellement porter leurs fruits
pour résoudre le problème présenté en introduction. Ce projet aura permis, dans le temps imparti,
de proposer une première approche. Comme mentionné ci-dessous, parmi les possibilités figure le fait que cette approche
ne soit pas loin de celle qui marchera correctement !
